...
from keras.backend import tensorflow_backend as backend #注意（これがいる！！）
...
def predict_onm(file):
    ...
    #読み込んだモデルで予測を出す．
    Xmel = Xmel.reshape(Xmel.shape[0], Xmel.shape[1], Xmel.shape[2], 1)
    predict_nn = model.predict(Xmel, batch_size=None, verbose=0, steps=None)
